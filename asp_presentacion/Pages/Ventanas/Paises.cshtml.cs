...
namespace asp_presentacion.Pages.Ventanas
{
    ...
    {
        private IPaisesPresentacion? iPresentacion = null;

        public PaisesModel(IPaisesPresentacion iPresentacion)
        {
            try
            {
        ...
        [BindProperty] public Paises? Filtro { get; set; }
        ...
        public virtual void OnPostBtGuardar()
        {
            try
            {
                ...
                Task<Paises>? task = null;
                if (Actual!.Id == 0)
                {
                    task = this.iPresentacion!.Guardar(Actual!)!;
                    GuardarAuditoria("Gua", "Guardar", "Paises", Actual.Nombre, IndexModel.UsuarioGlobal, DateTime.Now);

                }
                else
                {
                    task = this.iPresentacion!.Modificar(Actual!)!;
                    GuardarAuditoria("Mod", "Modificar", "Paises", Actual.Nombre, IndexModel.UsuarioGlobal, DateTime.Now);
                }
                task.Wait();
                Actual = task.Result;
                Accion = Enumerables.Ventanas.Listas;
        ...
        public virtual void OnPostBtBorrar()
        {
            try
            {
                ...
                GuardarAuditoria("Bor", "Borrar", "Paises", Actual.Nombre, IndexModel.UsuarioGlobal, DateTime.Now);
